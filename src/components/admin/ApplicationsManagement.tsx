...
export function ApplicationsManagement() {
  ...
  const changeAdoptionStatus = (app: AdoptionApplication, status: ApplicationStatus) => {
    if (app.status === status || !confirmStatusChange(`la solicitud de ${app.applicant_name}`, adoptionStatusLabel[status].toLowerCase())) return;
  ...
  const changeVolunteerStatus = (vol: VolunteerApplication, status: VolunteerApplication['status']) => {
    if (vol.status === status || !confirmStatusChange(`la solicitud de ${vol.full_name}`, volunteerStatusLabel[status].toLowerCase())) return;
...
function AdoptionList({ applications, total, isUpdating, onStatusChange }: { applications: AdoptionApplication[]; total: number; isUpdating: boolean; onStatusChange: (app: AdoptionApplication, status: ApplicationStatus) => void }) {
  ...
  return <div className="space-y-4 divide-y divide-[var(--color-border)]">{applications.map((app) => { const cleanPhone = app.applicant_phone.replace(/\D/g, ''); const whatsappUrl = cleanPhone ? `https://wa.me/${cleanPhone}?text=${encodeURIComponent(`Hola ${app.applicant_name}, nos comunicamos de AdoptaME sobre tu solicitud de adopción.`)}` : null; return <div key={app.id} className="space-y-3 pt-4 first:pt-0"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-heading text-lg font-bold">{app.applicant_name}</h3><p className="text-xs font-semibold text-[var(--color-primary)]">Interesado en: {app.animal?.name ?? 'Perro rescatado'}</p></div><Badge variant={getBadgeVariant(app.status)}>{adoptionStatusLabel[app.status]}</Badge></div><div className="grid grid-cols-1 gap-2 rounded-xl bg-[var(--color-background)] p-3 text-xs text-[var(--color-muted-foreground)] sm:grid-cols-3"><span className="flex items-center gap-1.5 break-all"><Mail className="h-3.5 w-3.5 shrink-0" />{app.applicant_email}</span>{whatsappUrl ? <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 font-semibold text-emerald-600 hover:underline"><Phone className="h-3.5 w-3.5 shrink-0" />{app.applicant_phone} · WhatsApp</a> : <span className="flex items-center gap-1.5"><Phone className="h-3.5 w-3.5 shrink-0" />{app.applicant_phone}</span>}<span className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 shrink-0" />{app.applicant_address}</span></div><p className="border-l-2 border-[var(--color-primary)] pl-3 text-xs italic leading-relaxed">“{app.reason}”</p><div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-[var(--color-muted-foreground)]">Recibida: {formatDateShort(app.created_at)}</span><div className="flex flex-wrap gap-2">{app.status !== 'approved' && <Button size="sm" variant="outline" className="text-emerald-600" disabled={isUpdating} onClick={() => onStatusChange(app, 'approved')}><CheckCircle2 className="mr-1 h-4 w-4" /> Aprobar</Button>}{app.status !== 'rejected' && <Button size="sm" variant="ghost" className="text-rose-600" disabled={isUpdating} onClick={() => onStatusChange(app, 'rejected')}><XCircle className="mr-1 h-4 w-4" /> Rechazar</Button>}{app.status === 'pending' && <Button size="sm" variant="secondary" disabled={isUpdating} onClick={() => onStatusChange(app, 'under_review')}>Pasar a revisión</Button>}</div></div></div>; })}</div>;
}
...
function VolunteerList({ applications, total, isUpdating, onStatusChange }: { applications: VolunteerApplication[]; total: number; isUpdating: boolean; onStatusChange: (vol: VolunteerApplication, status: VolunteerApplication['status']) => void }) {
  ...
  return <div className="space-y-4 divide-y divide-[var(--color-border)]">{applications.map((vol) => <div key={vol.id} className="space-y-3 pt-4 first:pt-0"><div className="flex flex-wrap items-center justify-between gap-2"><div><h3 className="font-heading text-base font-bold">{vol.full_name}</h3><p className="text-xs text-[var(--color-muted-foreground)]">{vol.area_of_interest} · {vol.availability}</p></div><Badge variant={getBadgeVariant(vol.status)}>{volunteerStatusLabel[vol.status]}</Badge></div><div className="grid grid-cols-1 gap-2 rounded-xl bg-[var(--color-background)] p-3 text-xs text-[var(--color-muted-foreground)] sm:grid-cols-3"><span className="break-all">✉️ {vol.email}</span><span>📞 {vol.phone}</span><span>🗓️ {vol.availability}</span></div>{vol.experience && <p className="text-xs leading-relaxed">Experiencia: {vol.experience}</p>}<div className="flex flex-col gap-2 pt-1 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs text-[var(--color-muted-foreground)]">Recibida: {formatDateShort(vol.created_at)}</span><select aria-label={`Estado de ${vol.full_name}`} value={vol.status} disabled={isUpdating} onChange={(event) => onStatusChange(vol, event.target.value as VolunteerApplication['status'])} className="h-9 rounded-lg border border-[var(--color-input)] bg-transparent px-2 text-xs sm:w-auto"><option value="pending">Pendiente</option><option value="contacted">Contactado</option><option value="active">Activo</option><option value="archived">Archivado</option></select></div></div>)}</div>;
}
