...
const PRODUCTS = [
  { name: 'Camiseta AdoptaME', type: 'Ropa solidaria', price: '$25', image: 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&q=80&w=900', color: 'coral' },
  { name: 'Pañuelo “ME eligieron”', type: 'Para tu mejor amigo', price: '$12', image: 'https://images.unsplash.com/photo-1583337130417-3346a1be7dee?auto=format&fit=crop&q=80&w=900', color: 'yellow' },
  { name: 'Tote bag AdoptaME', type: 'Uso diario', price: '$18', image: 'https://images.unsplash.com/photo-1597484662317-9bd7bdda7781?auto=format&fit=crop&q=80&w=900', color: 'cream' },
];
  ...
  return (
    <div className="pt-8">
      ...
      <section className="py-20 max-w-7xl mx-auto px-5 sm:px-8 lg:px-10">
        ...
        <div className="grid md:grid-cols-3 gap-7">{PRODUCTS.map((product) => <article key={product.name} className="group"><div className={`relative rounded-[1.5rem] overflow-hidden aspect-square ${product.color === 'coral' ? 'bg-[#f0644a]' : product.color === 'yellow' ? 'bg-[#ffcf5a]' : 'bg-[#ede5da]'}`}><img src={product.image} alt={product.name} className="w-full h-full object-cover mix-blend-multiply opacity-90 group-hover:scale-105 transition-transform duration-500" /><div className="absolute top-4 left-4 rounded-full bg-[#fffdf9] px-3 py-1 text-xs font-bold text-[#171717]">AdoptaME</div></div><div className="pt-4"><div className="flex justify-between gap-3"><div><p className="font-heading text-xl font-bold">{product.name}</p><p className="text-sm text-[#6e6a64] mt-1">{product.type}</p></div><span className="font-bold text-[#f0644a]">{product.price}</span></div><Button onClick={() => addProduct(product)} className="mt-4 w-full bg-[#171717] hover:bg-[#333] text-white" variant="default">{added.includes(product.name) ? <><Check /> Pedido enviado</> : <><ShoppingBag /> Lo quiero</>}</Button></div></article>)}</div>
      </section>

      <section className="bg-[#ede5da] py-16"><div className="max-w-5xl mx-auto px-5 sm:px-8 grid sm:grid-cols-3 gap-8 text-center"><div><div className="mx-auto w-11 h-11 rounded-full bg-[#fffdf9] flex items-center justify-center text-[#f0644a]"><Heart className="fill-current h-5 w-5" /></div><h3 className="font-heading font-bold mt-4">Compra con causa</h3><p className="text-sm text-[#6e6a64] mt-2">El 100% de la utilidad se destina al rescate.</p></div><div><div className="mx-auto w-11 h-11 rounded-full bg-[#fffdf9] flex items-center justify-center text-[#f0644a] font-bold">2</div><h3 className="font-heading font-bold mt-4">Recibe confirmación</h3><p className="text-sm text-[#6e6a64] mt-2">Te escribimos para confirmar talla, envío y pago.</p></div><div><div className="mx-auto w-11 h-11 rounded-full bg-[#fffdf9] flex items-center justify-center text-[#f0644a]"><Sparkles className="h-5 w-5" /></div><h3 className="font-heading font-bold mt-4">Comparte el mensaje</h3><p className="text-sm text-[#6e6a64] mt-2">Cada producto abre una conversación sobre adopción.</p></div></div></section>
