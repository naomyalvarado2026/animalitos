...
export function HomePage() {
  ...
  return (
    <div className="overflow-hidden">
      ...
      <section className="py-20 lg:py-28 max-w-7xl mx-auto px-5 sm:px-8 lg:px-10"><div className="flex flex-col md:flex-row md:items-end justify-between gap-5 mb-10"><div><p className="text-[#f0644a] font-bold text-sm uppercase tracking-[.15em]">Historias que empiezan aquí</p><h2 className="font-heading text-4xl sm:text-5xl font-extrabold tracking-[-.05em] mt-3">Ellos tienen algo que decirte.</h2></div><Link to="/adopta" className="font-bold text-[#f0644a] inline-flex items-center gap-2">Ver todos los perritos <ArrowRight className="h-4 w-4" /></Link></div>{featuredDogs.length ? <div className="grid md:grid-cols-3 gap-6">{featuredDogs.map((dog, i) => <motion.article key={dog.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * .08 }} className="group"><Link to={`/adopta/${dog.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}`} className="block"><div className="rounded-[1.5rem] overflow-hidden bg-[#ede5da] aspect-[4/4.6]">{dog.main_image_url ? <img src={assetUrl(dog.main_image_url)} alt={`${dog.name}, perrito en adopción`} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /> : <div className="h-full flex items-center justify-center text-6xl">🐶</div>}</div><div className="pt-4"><div className="flex items-center justify-between"><h3 className="font-heading text-2xl font-extrabold">{dog.name}</h3><span className="text-xs text-[#6e6a64]">{dog.age_months < 12 ? `${dog.age_months} meses` : `${Math.floor(dog.age_months / 12)} años`} · {dog.size}</span></div><p className="mt-2 text-[#6e6a64] leading-relaxed"><span className="text-[#f0644a] font-extrabold">ME</span> llamo {dog.name}, {dog.description || 'y estoy esperando una familia responsable.'}</p></div></Link></motion.article>)}</div> : <div className="rounded-3xl border border-dashed border-[var(--color-border)] p-10 text-center text-[var(--color-muted-foreground)]">Pronto publicaremos nuevos perritos disponibles. <Link to="/adopta" className="font-bold text-[#f0644a]">Conoce el proceso de adopción</Link>.</div>}</section>
