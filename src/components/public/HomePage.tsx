...
import { ArrowRight, CalendarDays, Check, HandHeart, Heart, PawPrint, ShieldCheck, Sparkles, Stethoscope, Users, Home as HomeIcon } from 'lucide-react';
...
function animalSlug(animal: Animal) { return animal.adoption_slug || animal.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''); }
...
const helpWays = [
  ...
  { title: 'Apadrinar', text: 'Acompaña el presente de un perro mientras llega su familia.', href: '/apadrina', icon: HandHeart, color: 'bg-[#f7b6cb] text-[#171717]' },
...
export function HomePage() {
  ...
  return <div className="overflow-hidden">
    ...
    <PackMosaic />
    <section className="bg-[#ede5da] py-20 text-[#171717]"><div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10"><div className="mb-10 max-w-2xl"><p className="text-sm font-bold uppercase tracking-[.15em] text-[#f0644a]">Hay muchas formas de cambiar una vida</p><h2 className="mt-3 font-heading text-4xl font-extrabold tracking-[-.05em] sm:text-5xl">Elige cómo quieres ser parte.</h2></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{helpWays.map(({ title, text, href, icon: Icon, color }) => <Link key={title} to={href} className={`group min-h-56 rounded-[1.5rem] p-5 transition hover:-translate-y-1 hover:shadow-xl ${color}`}><Icon className="h-7 w-7" /><div className="mt-12"><h3 className="font-heading text-2xl font-extrabold">{title}</h3><p className="mt-2 text-sm leading-relaxed opacity-80">{text}</p><span className="mt-4 inline-flex items-center gap-2 text-sm font-bold">Conocer más <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div></Link>)}</div></div></section>
