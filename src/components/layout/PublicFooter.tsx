import { Link } from 'react-router-dom';
import { Heart, Mail, MapPin } from 'lucide-react';
import { Separator } from '@/components/ui/separator';
import { usePublicSettings } from '@/lib/publicSettings';

const QUICK_LINKS = [
  { label: 'Inicio', href: '/' },
  { label: 'Adoptar', href: '/adopta' },
  { label: 'Cómo funciona', href: '/como-funciona' },
  { label: 'Tienda solidaria', href: '/tienda' },
  { label: 'Santuario', href: '/santuario' },
  { label: 'Recursos educativos', href: '/recursos' },
  { label: 'Nosotros', href: '/nosotros' },
  { label: 'Preguntas Frecuentes', href: '/faq' },
  { label: 'Transparencia', href: '/transparencia' },
  { label: 'Donaciones', href: '/donaciones' },
  { label: 'En memoria de', href: '/en-memoria' },
  { label: 'Contacto', href: '/contacto' },
];

const SOCIAL_LINKS = [
  {
    label: 'Facebook',
    key: 'social_facebook',
    svg: (
      <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
      </svg>
    ),
  },
  {
    label: 'Instagram',
    key: 'social_instagram',
    svg: (
      <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
      </svg>
    ),
  },
  {
    label: 'Twitter / X',
    key: 'social_twitter',
    svg: (
      <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
      </svg>
    ),
  },
  {
    label: 'TikTok',
    key: 'social_tiktok',
    svg: <span className="text-xs font-black">TikTok</span>,
  },
];

export function PublicFooter() {
  const { data: settings } = usePublicSettings(SOCIAL_LINKS.map(({ key }) => key));
  const socialLinks = SOCIAL_LINKS
    .map((link) => ({ ...link, href: settings?.[link.key]?.trim() ?? '' }))
    .filter((link) => link.href);

  return (
    <footer className="bg-[var(--color-card)] border-t border-[var(--color-border)] mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {/* Brand */}
          <div className="space-y-4">
            <Link to="/" className="flex items-center gap-2.5">
              <span className="font-heading text-xl tracking-[-0.05em] font-extrabold">Adopta<span className="text-[var(--color-primary)]">ME</span></span>
            </Link>
            <p className="text-sm text-[var(--color-muted-foreground)] leading-relaxed">
              Rescatamos, cuidamos y buscamos un hogar para perros que merecen una segunda oportunidad.
              Tu apoyo hace posible cada historia feliz. 🐾
            </p>
            {/* Social */}
            <div className="flex items-center gap-3 pt-1">
              {socialLinks.map(({ svg, href, label }) => (
                <a
                  key={label}
                  href={href}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={label}
                  className="w-9 h-9 rounded-lg border border-[var(--color-border)] flex items-center justify-center text-[var(--color-muted-foreground)] hover:text-[var(--color-primary)] hover:border-[var(--color-primary)] hover:bg-[var(--color-accent)] transition-colors"
                >
                  {svg}
                </a>
              ))}
            </div>
          </div>

          {/* Quick Links */}
          <div className="space-y-4">
            <h3 className="font-heading font-semibold text-sm uppercase tracking-wider text-[var(--color-muted-foreground)]">
              Navegación
            </h3>
            <ul className="space-y-2">
              {QUICK_LINKS.map((link) => (
                <li key={link.href}>
                  <Link
                    to={link.href}
                    className="text-sm text-[var(--color-foreground)] hover:text-[var(--color-primary)] transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact info */}
          <div className="space-y-4">
            <h3 className="font-heading font-semibold text-sm uppercase tracking-wider text-[var(--color-muted-foreground)]">
              Contáctanos
            </h3>
            <ul className="space-y-3">
              <li className="flex items-start gap-2.5 text-sm text-[var(--color-foreground)]">
                <MapPin className="h-4 w-4 mt-0.5 text-[var(--color-primary)] shrink-0" />
                <span>Visitas con cita previa · consulta disponibilidad</span>
              </li>
              <li className="flex items-center gap-2.5 text-sm">
                <Mail className="h-4 w-4 text-[var(--color-primary)] shrink-0" />
                <Link to="/contacto" className="hover:text-[var(--color-primary)] transition-colors">Escríbenos desde el formulario</Link>
              </li>
            </ul>

            <Link
              to="/donaciones"
              className="inline-flex items-center gap-2 mt-2 text-sm font-semibold text-[var(--color-primary)] hover:underline"
            >
              <Heart className="h-4 w-4" />
              Quiero hacer una donación
            </Link>
          </div>
        </div>

        <Separator className="my-8" />

        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[var(--color-muted-foreground)]">
          <span>© {new Date().getFullYear()} AdoptaME. Todos los derechos reservados.</span>
          <span className="flex items-center gap-1">
            Hecho con <Heart className="h-3 w-3 text-red-500 fill-red-500" /> para cada patita
          </span>
        </div>
      </div>
    </footer>
  );
}
