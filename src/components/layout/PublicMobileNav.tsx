...
export function PublicMobileNav() {
  ...
  useEffect(() => {
    const closeOnEscape = (event: KeyboardEvent) => event.key === 'Escape' && setIsMoreOpen(false);
  ...
  return (
    <>
      {isMoreOpen && (
        <div className="fixed inset-0 z-[65] md:hidden" role="presentation" onClick={() => setIsMoreOpen(false)}>
          <div className="absolute inset-0 bg-black/35" />
          <div className="absolute left-3 right-3 bottom-[calc(4.9rem+env(safe-area-inset-bottom))] rounded-3xl border border-[var(--color-border)] bg-[var(--color-card)] p-3 shadow-2xl" role="dialog" aria-modal="true" aria-label="Más opciones" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-center justify-between px-3 py-2"><p className="font-heading font-bold">Más de AdoptaME</p><button type="button" onClick={() => setIsMoreOpen(false)} className="mobile-touch-target rounded-xl" aria-label="Cerrar más opciones"><X className="h-5 w-5 mx-auto" /></button></div>
            <div className="grid grid-cols-2 gap-2 mt-2">
              {MORE_ITEMS.map((item) => <Link key={item.href} to={item.href} onClick={() => setIsMoreOpen(false)} className="rounded-2xl bg-[var(--color-background)] px-3 py-3 text-sm font-semibold hover:bg-[var(--color-accent)]">{item.label}</Link>)}
            </div>
          </div>
        </div>
      )}
      <nav className="fixed bottom-0 left-0 right-0 z-[60] md:hidden border-t border-[var(--color-border)] bg-[var(--color-card)]/95 shadow-[0_-8px_24px_rgba(23,23,23,.12)] backdrop-blur-md pb-[env(safe-area-inset-bottom)]" aria-label="Navegación móvil principal">
        <div className="grid grid-cols-5 h-[4.35rem] max-w-lg mx-auto">
          ...
          <button type="button" onClick={() => setIsMoreOpen((open) => !open)} aria-expanded={isMoreOpen} className={cn('flex flex-col items-center justify-center gap-1 text-[.68rem] font-semibold mobile-touch-target', isMoreOpen ? 'text-[var(--color-primary)]' : 'text-[var(--color-muted-foreground)]')}><Menu className="h-5 w-5" strokeWidth={2.2} /><span>Más</span></button>
        </div>
      </nav>
    </>
  );
